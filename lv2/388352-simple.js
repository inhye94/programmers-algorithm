...
function solution(n, q, ans) {
  const TOTAL_COUNT = 5;
  const candidates = getCandidates(n, q, ans); // 숫자 범위 줄이기 - 최적화
  let result = 0;

  // 정수 조합 개수 찾기
  const combinations = dfs(candidates, TOTAL_COUNT);

  for (let comb of combinations) {
    if (isValid(comb, q, ans)) result++;
  }
...
// 유효성 확인
function isValid(combination, q, ans) {
